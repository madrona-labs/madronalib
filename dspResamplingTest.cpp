...
TEST_CASE("madronalib/resampling/nyquist_rejection", "[resampling]")
{
  Upsampler2x<float> up;
  Downsampler2x<float> down;
  
  // signal near Nyquist: omega = 0.49
  float phase = 0.f;
  ...
  // near-Nyquist content should be heavily attenuated after roundtrip
  // (rejected by both the upsample and downsample filters)
  REQUIRE(rms(result) < 0.01f);
}
